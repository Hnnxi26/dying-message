...
export const criminals=['모델','음악가','바텐더','요리사','경찰','과학자','복서','마약상','시인','영화감독','건축가','미용사','의사','사기꾼','가수','군인','화가','야구선수','신문기자','아이돌','살인마','변호사','경호원'];
...
export const draftTileIds=(r:Room)=>{ensureDraftHints(r);return new Set(Object.values(r.draftHints).flat().map(t=>t.id))}
...
export function startCountdown(room:Room,durationMs=4000){
  ...
  Object.values(room.teams).forEach(team=>{
    if(team.status!=='gameover'&&team.status!=='success'){
      team.notice=room.round===4?'FINAL ROUND 시작 대기':`ROUND ${room.round} 시작 대기`;
    }
  });
}
